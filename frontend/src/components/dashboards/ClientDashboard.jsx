    ...
    try {
      ...
      if (res.ok) {
        ...
        if (resData.project) {
          ...
          setClientProjects(prev => [persistedProj, ...prev.filter(p => p.id !== newProj.id)]);
        }
      }
    } catch (err) {
      console.warn('Backend sync warning:', err);
                          ...
                          <div className="space-y-2 min-w-0 flex-1">
                            <div className="flex items-center space-x-3 flex-wrap gap-y-1">
                              ...
                              {(() => {
                                ...
                                if (appStatus === 'Pending Review') {
                                  return (
                                    <span className="px-3 py-1 rounded-full text-xs font-extrabold bg-amber-100 text-amber-900 border border-amber-300 flex items-center space-x-1">
                                  ...
                                  return (
                                    <span className="px-3 py-1 rounded-full text-xs font-extrabold bg-rose-100 text-rose-900 border border-rose-300 flex items-center space-x-1">
                            ...
                            {(p.approval_status === 'Rejected' || p.approvalStatus === 'Rejected') && (
                              <div className="p-3.5 rounded-2xl bg-rose-50 border border-rose-200 text-xs space-y-1 my-2">
                                <div className="flex items-center space-x-1.5 text-rose-800 font-extrabold">
                                  <AlertCircle className="w-4 h-4 text-rose-600 shrink-0" />
                                  <span>Project Review Rejected by Admin</span>
                                </div>
                                <p className="text-slate-700 font-medium pl-5">
                                  <strong>Reason:</strong> {p.rejection_reason || p.rejectionReason || 'Does not meet platform quality standards.'}
                                </p>
                              </div>
                            )}

                            {(p.approval_status === 'Pending Review' || p.approvalStatus === 'Pending Review') && (
                              <div className="p-3 rounded-xl bg-amber-50/70 border border-amber-200/80 text-[11px] text-amber-900 font-semibold my-1 flex items-center space-x-2">
                                <Info className="w-3.5 h-3.5 text-amber-600 shrink-0" />
                                <span>Awaiting admin review. Once verified, this project will be automatically published for freelancers in Browse Jobs.</span>
                              </div>
                            )}
                ...
                <div>
                  <div className="flex items-center space-x-3">
                    {contractCode ? (
                      ...
                      <span className="font-extrabold text-slate-500 text-xs px-3 py-1 rounded-xl bg-slate-100 border border-slate-200">
                        Contract: N/A
                      </span>
                    )}
                    <span className={`px-3 py-1 rounded-full text-xs font-extrabold border ${
                      appStatus === 'Pending Review' ? 'bg-amber-100 text-amber-900 border-amber-300' :
                      appStatus === 'Rejected' ? 'bg-rose-100 text-rose-900 border-rose-300' :
                      p.status === 'Completed' ? 'bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-blue-100 text-blue-700 border-blue-200'
                    }`}>
                      {appStatus === 'Pending Review' ? 'Pending Admin Review' : (appStatus === 'Rejected' ? 'Rejected by Admin' : (p.status || 'Open for Bids'))}
                    </span>
                  </div>
                  <h3 className="text-xl sm:text-2xl font-extrabold text-slate-900 mt-2 tracking-tight">
                    {formatTitle(p.title)}
              ...
              {appStatus === 'Pending Review' && (
                <div className="p-4 rounded-2xl bg-amber-50 border border-amber-200 text-xs text-amber-900 space-y-1">
                  ...
                  <p className="font-medium text-amber-800/90">
                    This newly posted project is currently undergoing admin security & quality review. Once approved, it will automatically become visible to freelancers in the Browse Jobs marketplace.
                  </p>
                </div>
              )}
              ...
              {appStatus === 'Rejected' && (
                <div className="p-4 rounded-2xl bg-rose-50 border border-rose-200 text-xs text-rose-900 space-y-1">
                  <div className="flex items-center space-x-2 font-extrabold text-rose-800">
                    <AlertCircle className="w-4 h-4 text-rose-600 shrink-0" />
                    <span>Project Rejected by Admin</span>
                  </div>
                  <p className="font-medium text-slate-700">
                    <strong>Rejection Reason:</strong> {p.rejection_reason || p.rejectionReason || 'Does not meet platform quality standards.'}
                  </p>
                </div>
              )}
              ...
              <div className="space-y-3 text-xs">
                ...
                <div className="space-y-2">
                  <div className="p-3 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center justify-between">
                    <div>
                      <p className="font-bold text-slate-900">1. Project Posted & Saved</p>
                      <p className="text-slate-700 dark:text-slate-300 text-xs">Posted on {p.postedDate || 'Recently'}. Budget: {formatCurrency(p.budget)}.</p>
                    </div>
                  ...
                  <div className="p-3 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center justify-between">
                    <div>
                      <p className="font-bold text-slate-900">2. Admin Verification</p>
                      <p className="text-slate-700 dark:text-slate-300 text-xs">
                        {appStatus === 'Approved' ? 'Verified & approved by platform admin.' : (appStatus === 'Rejected' ? 'Rejected by admin review.' : 'Awaiting admin approval in verification queue.')}
                      </p>
                    </div>
                    <span className={`font-bold text-xs ${appStatus === 'Approved' ? 'text-emerald-600' : (appStatus === 'Rejected' ? 'text-rose-600' : 'text-amber-600')}`}>
                      {appStatus === 'Approved' ? 'Completed' : (appStatus === 'Rejected' ? 'Rejected' : 'Pending')}
                    </span>
                  </div>

                  <div className="p-3 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center justify-between">
                    <div>
                      <p className="font-bold text-slate-900">3. Proposal Bidding & Hiring</p>
                      <p className="text-slate-700 dark:text-slate-300 text-xs">
                        {hiredName ? `${hiredName} hired for project deliverables.` : `${pAppCount} proposal(s) submitted.`}
                      </p>
                    </div>
                    <span className={`font-bold text-xs ${hiredName ? 'text-emerald-600' : (appStatus === 'Approved' ? 'text-blue-600' : 'text-slate-400')}`}>
                      {hiredName ? 'Completed' : (appStatus === 'Approved' ? 'Open for Bids' : 'Pending Approval')}
                    </span>
                  </div>

                  <div className="p-3 rounded-xl bg-slate-50 border border-slate-200/80 flex items-center justify-between">
                    <div>
                      <p className="font-bold text-slate-900">4. Contract & Escrow Activation</p>
                      <p className="text-slate-700 dark:text-slate-300 text-xs">
                        {contractCode ? `Contract ${contractCode} active with funded escrow.` : 'Contract & Escrow will be created upon hiring.'}
                      </p>
                    </div>
                    <span className={`font-bold text-xs ${contractCode ? 'text-emerald-600' : 'text-slate-400'}`}>
                      {contractCode ? 'Completed' : 'Pending'}
                    </span>
                  </div>
                </div>
              </div>
